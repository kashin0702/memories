...
​	1.当前时间<早上6点 5-27至5-28

2. 当前时间>早6点  5-28至5-28
...
//定时器刷新方法
  interval: function(orderList){
    ...
    timer=setInterval(function(){
        //遍历orderList,拿到里面的wait_time进行格式化赋值，并对该值++，下一次调用时就会更新
...
checkboxChange(e){
    ...
    this.manual_order_discount_ratio_index = 0
    //不使用优惠券情况
    //使用for循环 return才有效(break也行)， forEach中return无效
...
must_order: 1  //代表这个菜必须点

must_order_according : 'nums'  //代表必点菜数量根据菜的数量叠加

total_must_order: 2  	//代表还需点几个必点菜
...
获取订单详情接口**order/buyNow**，其中返回已保存的地址address

address/create页

接口：**address/add**  添加新收货地址

address/index 页

接口: **address/lists** 返回所有已保存的地址
