...
function initComputed(vm) {
    ...
    for (const key in computed) {
        ...
        const getter = typeof userDef === 'function' ? userDef : userDef.get // 处理computed是方法或者对象
...
function defineComputed(target, key, userDef) {
    ...
    //包装计算属性方法名，给 vm 上添加 computed 方法，渲染时访问的computed属性就是sharedPropertyDefinition.get
...
export default class VueRouter {
 // VueRouter的静态方法，在Vue.use中被调用
  static install(Vue) {
    ...
    //3、核心步骤：在beforeCreate回调函数内进行router的获取和初始化，把创建Vue实例时候传入的router对象注入到Vue实例上。
    _Vue.mixin({
  ...
  // init中会调用根据mode创建的hashHistory或HTMLHistory
  init(Vue) {
   ...
   Vue.component("router-link", {
     ...
     Vue.component("router-view", {
		  render(h) {
            ...
            //获取路由记录对象. matched是当前url匹配的一个数组，元素是路由规则对象，有component属性
...
export default function createRoute(record, path) {
  const matched = [];
  while (record) {
    matched.unshift(record); // 理解：和上面route.matched获取组件的顺序对应
