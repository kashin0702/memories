...
3. desc 属性描述符
   ...
   **访问器描述符**是由 getter/setter 函数对描述的属性。**描述符只能是这两种类型之一，不能同时为两者。**
...
// 监听多个对象
watch(() => [formData.custProp, formData.customerLevel], ([newVal1, newVal2], [oldVal1, oldVal2]) => {
  console.log('监听======')
...
返回一个给定对象自身的所有可枚举属性值的数组，值的顺序与使用[`for...in`](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Statements/for...in)循环的顺序相同 ( 区别在于 for-in 循环枚举原型链中的key )。
...
scss引入别名路径时使用~@前缀，为了和@区分，因为scss定义变量使用@符号
...
inline元素或table-cell元素，包括 `span` , `img` , `input` , `button` , `td` 以及通过display改变了显示水平为inline水平或者table-cell的元素

**注意：纯文字属于匿名inline元素**

**img元素的内容不是文字，不存在内容区域， 因此对于img元素可以把内容区域看作是元素自身**

内容区域受font-size和font-family影响



对齐概念：

​	**vertical-align的默认值`baseline`的含义是将元素的基线与父元素的基线对齐。**

​	**vertical-align: middle 官方解释**

​		**让子元素盒子垂直中点与父盒子的baseline + 字母 x 高度的一半对齐。**

​	**简单来说，基线就是元素内文本的下沿线**

元素基线的位置确认(分情况)：

​	inline元素: 内联元素的baseline是里面文本字母 *x* 的下边缘线

​	inline-block元素: 对于有内容的inline-block，其基线为最后一行文本基线所在的位置，而对于空白的inline-block，其基线为margin-	      bottom边缘所在位置，即底部边缘

父元素基线的位置确认：

​	**父元素基线的位置由子元素基线位置确定（多个子元素时，由距离父元素顶部最远的决定）**
...
<body>
    <div class="all">
        <!-- 行内文字abcdef会和box1设置的vertical-align对齐 -->
...
![img](D:\typora-img\v2-efe30b60d553cb6237f9c0cbdebb240b_720w.webp)

**内联模型(line-box)的高度不是文字内容决定的，是由行高决定的**

**line-height不会改变元素本身内容高度，仅增加上下行距，撑开了行内盒子模型**
...
当line-height:0  内联盒子高度就为0，当font-size:0 line-height不为0，模型有高度，但文字消失





line-height可以设置1.5 1.5em 150% 比例值或具体值， 可被子元素继承

1.5em和150%的计算值会被子元素直接继承，而1.5会根据子元素字体大小重新计算后继承（推荐使用1.5不带单位）



内联盒模型中有4条线：顶线top-line, 中线middle-line, 基线base-line, 底线bottom-line，**浏览器水平方向默认按基线对齐**
...
每一行就是一个行框盒子，由多个内联盒子组合而成的一行 就会形成一个**行框盒子**

**注意: 行框高度等于本行框内部,所有行内框(内联盒子)中,最大的那个行内框(内联盒子)的值! **

**重点: 是以带有行高值最大的行内框(内联元素)为基准来最终计算行框的高度，其他行内框(内联元素)采用自己的对齐方式向基准对齐!**

```html
<!-- 这一整个就是行框盒子，不带标签的“普通文字”、“标签”是匿名内联盒子， em标签内是内联盒子  -->
<p> 普通的文字<em>我是斜体</em> 标签 </p>
```
...
**prototype: 每个函数都有一个 `prototype` 属性，指向一个对象（称为原型对象），该对象包含可以被实例继承的属性和方法**
...
proto属性指向其构造函数的prototype(实例对象继承的本质)
...
**`Object.getPrototypeOf()`** **静态方法**返回指定对象的原型（即内部 `[[Prototype]]` 属性的值）。
...
**`isPrototypeOf()`** 用于检查调用对象是否存在于传入对象的原型链中。

`isPrototypeOf()` 与 [`instanceof`](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Operators/instanceof) 运算符不同。在表达式 `object instanceof AFunction` 中，会检查 `object` 的原型链是否与 `AFunction.prototype` 匹配，而不是与 `AFunction`本身匹配。
...
所有对象（包括函数）最终都继承自Object

所有函数都是 `Function` 的实例

```js
/*
1.Function instanceof Object 为 true，是因为 Function 是一个对象，其原型链经过 Function.prototype → Object.prototype → null，所以它是 Object 的“实例”（从原型链角度看）。
2.Object instanceof Function 为 true，是因为 Object 是一个函数，由 Function 构造。
*/
...
Js中Object属于基类，Date, Number, Function可以看作派生类

特殊的是Obejct又是Function这个派生类的实例对象

```js
// Function的__proto__和prototype都指向Function的原型，符合所有函数的__proto__指向Fucntion.prototype
Function.__proto__ === Function.prototype

// 所有构造函数都是Function类的实例, 包括Object
Array.__proto__ === Function.prototype
Object.__proto__ === Function.prototype
Array.constructor === Function
Object.constructor === Function

// 重要：Function的原型链指向Object的原型属性  Object.prototype是原型链终点
Function.prototype.__proto__ === Object.prototype
...
/* 
个人理解: Object.hasOwnProperty === Object.prototype.hasOwnProperty
通过原型链查看，hasOwnProperty挂在Object.prototype对象上，没有出现在Object对象，所以是通过原型链调用的
*/
Object.__proto__ === Function.prototype // Object的__proto__属性指向函数的原型
Function.prototype.__proto__ === Object.prototype // 所有对象的__proto__都指向Object.prototype
// 所以Object通过原型链能调到Object.prototype上的方法
```
...
export default {
    render(){
        ...
        <div>
            <h1></h1>
            <div>
                <!-- 把father的默认插槽放到son组件中，要包裹一个default插槽对象，在son中就会被执行 -->
                <son>{{ default: () => this.$slots.default() }}</son>
            </div>
        </div>
    }
}
...
- 使用 ?? 时，只有前者为 null 或者 undefined 时才会返回 后者;
- 使用 || 时，前者会先转化为布尔值判断，为true时返回前者 , false 返回后者
...
<template>
	<!--渲染recharge组件， 官方叫命名视图可以在一个界面展示多个router-view -->
...
**因为使用v-model,子组件中只能触发一个input事件,**事件名是唯一的,而.sync修饰符,它触发的时间名是`updata:属性名`,所以它有多个事件名,就可以使用多次
...
<head>
  ...
  <script>
    function handleFile(e) {
      var file = e.files[0]; // 原生input对象用e.files[0] 获取文件信息， 不是e.target.files
      var reader = new FileReader();
      var img = document.getElementByTagName("img")[0];
	  reader.readAsDataURL(file);
      reader.onload = function() {
        img.src = reader.result;  // 转成base64后的图片
      }
    }
  </script>
</head>
...
<head>
  ...
  <script>
    function handleFile(e) {
      ...
      img.src = blobUrl;
      img.onload = function() {
        URL.revokeObjectURL(blobUrl); //释放createObjectURL创建得对象
      }
    }
  </script>
</head>
...
参数：**blobparts** : 数组类型 数组中每项连在一起构成blob数据，由ArrayBuffer，ArrayBufferView，Blob，DOMString等对象构成

**options**: 可选项 字典格式类型 

可指定: 

1. **type**   默认值为`""`，代表被放入到blob中的数组内容的MIME类型（text/plain  text/html  text/javascript  application/json等等）
...
通过`URL.createObjectURL(blob或file)`创建

通常作为文件的下载地址和作为图片资源地址

**URL.createObjectURL(blob或file)可以获取当前文件的一个内存URL，每次生成都不同，也可以用来创建随机字符串**
...
```vue
<!-- 使用 $event 代替组件方法中原本的单个参数，再加上自己想要多传的参数即可 -->
<demo v-model="value" @change="onChange($event, 额外参数)">选择</demo>
...
<!-- 事件参数有多个，使用arguments代表多个参数 -->
<demo v-model="value" @change="onChange(...arguments, 额外参数)">选择</demo>

使用：

onChange （原参数1, 原参数2, 原参数3, ..., 额外参数）{
...
encodeURI编码后url仍可以使用，即如果还需要使用改URL进行网络传输则可以使用encodeURI。例如URL出现了中文，中文在网络中无法直接传输，则URL需要编码。
（事实上，我们在浏览器中输入一个含有中文参数的URL，在发出请求后，会自动使用encodeURI进行编码。所以有的带中文的url在浏览器可以使用，而在小程序中用不了，大概也是这个原因）

```js
encodeURI("http://www.cnblogs.com/season-huang/some other thing");
//encodeURI编码后会变为(空格被编码成了%20)  ==> "http://www.cnblogs.com/season-huang/some%20other%20thing";
//encodeURIComponent编码后会变为(连 "/" 都被编码了，整个URL已经没法用了) ==> "http%3A%2F%2Fwww.cnblogs.com%2Fseason-huang%2Fsome%20other%20thing"

```



### encodeURIComponent使用场景
...
encodeURI()主要用于整个URI,它不会对本身属于URI的特殊字符进行编码例如： ASCII字母、数字、~!@#$&*()=:/,;?+'

encodeURIComponent()可用于编码URI中的参数，encodeURIComponent方法不会对下列字符编码：ASCII字母、数字、~!*()'

encodeURIComponent解码的范围更广，encodeURI主要是对空格进行编码
...
```js
//第二个参数使用函数 该函数有多个入参 
// macth:匹配到的字符串 p1,p2,p3：匹配到的子字符串， offset：匹配到的索引起始位置  string:原字符串
...
**proxy_pass：代理地址末尾加''/''斜杠表示代理到绝对路径，最终请求路径不会带上/api， 末尾不加''/''则会带上/api**

```makefile
http {
	server { # 启动一个服务
		listen 8000; #监听服务器的8000端口 访问：http://服务器ip:8000端口
		server_name localhost; #本机访问localhost
		
		#根目录 访问根目录时返回前端资源  访问根目录，实际访问路径是root+location
		location / {
			root /dist;   #前端项目在文件系统中的路径
			index index.html index.htm;   #首页响应文件
		}
		 # 反向代理：匹配所有/api开头的地址，如需要访问xx.xx.xx.xx:9090端口的服务，则如下配置
		location /api {
			proxy_pass http://xx.xx.xx.xx:9090; #最后不加‘/’最终访问地址是http://xx.xx.xx.xx:9090/api
		}
	}
}
...
```makefile
#1.两个都带/   理解为去掉/api/，匹配后面的upload
location /api/ {
	# 访问地址www.test.com/api/upload --> http://127.0.0.1:8080/upload
	proxy_pass http://127.0.0.1:8080/;
}
#2.location不带/,   理解为去掉/api，匹配后面的/upload ，所以代理地址会多一个/
location /api {
	# 访问地址www.test.com/api/upload --> http://127.0.0.1:8080//upload
	proxy_pass http://127.0.0.1:8080/;
}
#3.proxy_pass不带/， 真实地址会带location匹配目录/api/
location /api/ {
	# 访问地址www.test.com/api/upload --> http://127.0.0.1:8080/api/upload
	proxy_pass http://127.0.0.1:8080;
}
#4.都不带/, 真实地址会带location匹配目录/api/
location /api {
	# 访问地址www.test.com/api/upload --> http://127.0.0.1:8080/api/upload
	proxy_pass http://127.0.0.1:8080;
}

# 理解：只要proxy_pass中带了地址，location都会被去掉，匹配location后面的路径
#5.同1，但proxy_pass带地址
location /api/ {
	# 访问地址www.test.com/api/upload --> http://127.0.0.1:8080/server/upload
	proxy_pass http://127.0.0.1:8080/sever/;
}
#6. 同2，但proxy_pass带地址， 去掉/api匹配了后面的/upload, 所以代理地址会多一个/
location /api {
	# 访问地址www.test.com/api/upload --> http://127.0.0.1:8080/server//upload
	proxy_pass http://127.0.0.1:8080/server/;
}
#7.同3，但proxy_pass带地址，去掉/api/匹配了upload, 代理地址就连起来了
location /api/ {
	# 访问地址www.test.com/api/upload --> http://127.0.0.1:8080/serverupload
	proxy_pass http://127.0.0.1:8080/server;
}
#8.同4，但proxy_pass带地址，去掉了/api匹配了/upload
location /api {
	# 访问地址www.test.com/api/upload --> http://127.0.0.1:8080/server/upload
	proxy_pass http://127.0.0.1:8080/server;
}
...
　　当nginx配置文件proxy_pass后边的URL不带"/"时，连同匹配到的`/poss/`路径，一起进行反向代理,代理到后端的路径为：

http://127.0.0.1:8080/poss/abc/bill.html
...
location /test01 {
    ...
    location /test02/ {
　　　　#注意如果访问的是http://localhost/test02    则不会匹配这一项，因为这个location至少要匹配/test02/
        proxy_pass http://127.0.0.1:8888;
        # 访问http://localhost/test02/123  ==> 127.0.0.1:8888/test02/123
    }

    location /test03/ {
        # 注意不会匹配localhost/test03，理由同上
        proxy_pass http://127.0.0.1:8888/;
        # 访问http://localhost/test03/     ==>  127.0.0.1:8888/
        # 访问：http://localhost/test03/123  ==>  127.0.0.1:8888/123
    }

    location /test04 {
        proxy_pass http://127.0.0.1:8888/;
        # 访问http://localhost/test04aaa   ==>  127.0.0.1:8888/aaa
        # 访问http://localhost/test04/123  ==>  127.0.0.1:8888//123   
    }
 ```
...
![image-20240122152344872](D:\typora-img\image-20240122152344872.png)
...
请求URI带斜杠结尾，识别为目录，不带斜杠识别为文件，如访问domain.com/some-dir/，访问 /some-dir/ 时，服务器会自动去该目录下找对应的默认文件。如果访问 /some-dir 的话，服务器会先去找 some-dir 文件，找不到的话会将 some-dir 当成目录，重定向到 /some-dir/ ，去该目录下找默认文件

root和alias都用于指定web服务器的根目录，区别是如何处理URI

root: 当location是 / ，root是/dist  , location的路径会拼接到root之后 

前缀匹配：不加任何符号，匹配/xxx/开头的 

~开头表示执行正则匹配

最长前缀匹配：^~ 匹配/xxx/开头，一旦匹配成功，停止匹配其他location，

alias: location中的路径不会拼到alias的地址后面，会直接替换
...
server {
  ...
  # 请求路径为：http://localhost:8080/files/index.jpg    实际访问为：/home/index.jpg
  location ^~/files/ {
    alias /home/;
    index index.html index.htm;
  }
...
server {
  ...
  location ^~/my_demo/ {
    ...
    proxy_set_header X-Real-IP $remote_addr;
    proxy_set_header Host $http_host;
    root /home/web/;  # 实际请求：location会拼接到root之后，变成/home/web/my_demo/
    index index.html index.htm;
  }
}
```
...
scoped在渲染的时候，**如果组件内部还有子组件，只会在父组件，以及子组件的根元素加上`data-v-xxx`属性**
...
vue2中样式穿透三种写法： **>>>（只能在css编译器中使用）  /deep/  ::v-deep**



scoped会在选择器最后默认会加上当前组件的一个标识，比如[data-v-49729759]



**用了样式穿透后：**

**1.单独使用/deep/ 会编译成[data-v-xx]的属性选择器**

**2.在类选择器后面使用/deep/  会编译成.xx[data-v-xx] 的复合选择器**

**3.在deep之后的选择器最后就不会加上标识。**
...
/** 
    加了/deep/编译后，父级会加一个当前组件的data标识，且deep后的子选择器不会加data标识
    .cssdeep[data-v-xx] .cssdeepcom {
		background: blue;
    }
    没有/deep/编译后， scoped会自动在选择器最后加一个当前组件的data标识， 所以对子组件内部样式无法生效
    .cssdeep .cssdeepcom[data-v-xx] {
    	background: blue;
    }
 */
...
/** 
    组件的样式编译后会自动加一个data属性选择器标识
    .cssdeepcom[data-v-aa] {
		width: 100px;
  		height: 100px;
		background: red;
}*/    
...
- **index:** index.html存放位置
- **assetsRoot**:打包后文件存放的路径
- **assetsSubDirctory**：除了index.html之外的静态资源要存放的路径，一般可设置为static
- **assetsPublicPath**: 代表打包后，index.html里面引用资源的相对地址，
  - 如设置为/AAA   则index内引用的资源位置就是/AAA/static/xx，一般设置为/ 即根目录



### vue-cli 3.x 路径配置

- **publicPath:  '/'**   同assetsPublicPath
- **outputDir:  'dist'**   执行build时，生产环境构建文件的目录
- **assetDir: 'static'**     放置静态资源的目录（相对于outputDir）
...
console.log(import.meta); // { url: "file:///home/user/my-module.mjs" } 它返回一个带有url属性的对象，指明模块的基本 URL。也可以是外部脚本的 URL，还可以是内联脚本所属文档的 URL。
```
...
### import/export和require主要区别

import/export必须定义在模块顶层，不能写在代码中

require属于运行时动态加载模块，可以写在代码中
...
// 将一个数组合并到另一个数组中 扩展运算符只能用于具有迭代器接口的对象，也就是可以使用for...of循环的对象
let arr4 = [1, 2, 3];
let arr5 = [4, 5];
arr4.push(...arr5);
...
WebSocket是[基于HTTP协议](https://so.csdn.net/so/search?q=基于HTTP协议&spm=1001.2101.3001.7020)的，借助HTTP协议来完成握手
...
语法 ：string.charCodeAt(index)    // index代表要获取字符的位置，从0开始
...
![img](D:\typora-img\4cc6b55cbd806f9aa65ae0bfc4b25a5c.png)

ArrayBuffer比Blob更底层，直接操作二进制数据，Blob是一个封装好的二进制对象
...
Uint8Array是JavaScript中的一种类型化数组。它提供了一种用于表示8位无符号整数的固定长度的数组，**只能存放整数类型**

其他类型赋值会无效

Float32Array用来存储浮点类型数据的数组，**只能存储Number数字型数据**，其他数据不允许会赋值为NaN
...
DataView的类型如下：
Int8Array：8位有符号整数，每一项占1个字节。（-128~127）
Uint8Array：8位无符号整数，每一项占1个字节。（0~255）
Int16Array：16位有符号整数，每一项占2个字节。（-32768,32767）
Uint16Array：16位无符号整数，每一项占2个字节。（0~65535）
Int32Array：32位有符号整数，每一项占个字节。（-2147483648~2147483647）
Uint32Array：32位无符号整数，每一项占4个字节。（0~4294967295）
Float32Array：32位浮点数，每一项占4个字节。
Float64Array：64位浮点数，每一项占8个字节。

类型化视图一般也被称为类型化数组，因为它们除了元素必须是某种特定的数据类型外，与常规的数组无异。而且它们**都继承了DataView**。

ArrayBuffer对象并没有提供任何读写内存的方法，而是允许在其上方建立“视图”，从而插入与读取内存中的数据

![img](D:\typora-img\8a3ca0e53b9e0a59aaf0476adfb985fa.png)
...
// 字符 转 Uint8Array
let uint8Array = encoder.encode("你好啊") // 使用utf8编码将字符串转化为类型数组(字节数组)

// 生成一个9个字节长度的类型数组
 console.log(uint8Array) // [228, 189, 160, 229, 165, 189, 229, 149, 138, buffer: ArrayBuffer(9), byteLength: 9, byteOffset: 0, length: 9, Symbol(Symbol.toStringTag): 'Uint8Array']
...
```html
<!-- 为line标签指定一个id。而在需要使用的地方，通过<use>标签设置href属性为之前定义的id即可 -->
<svg>     
...
xlink:href的常用值是URL地址，指向SVG文档外部的资源, **SVG2弃用，建议直接使用href**
...
```js
// 本质是执行了入参中的install方法， 而install方法中可以给Vue.prototype添加一些东西，方便实例使用
...
function install(Vue) {
  ...
  beforeCreate: function beforeCreate () {
    if (isDef(this.$options.router)) {
      ...
      this._router = this.$options.router; // 真正传入的router实例，在生命周期钩子函数中才会获得
...
export function mapMenusToRoutes(userMenus: any[]): RouteRecordRaw[] {
  ...
  routeFiles.keys().forEach((key) => {
    ...
    const route = require('../router/main' + key.split('.')[1]) // 返回一个module，有一个default属性，就是这个route对象
...
### 路由守卫beforeEach中next()逻辑

在路由守卫当中,只有`next()`是放行(放你通过,不会在审核),而`next("/")` `next("/login")` `next({...to})` `next({...to,replace:true}` 等,都是中断当前的全局前置守卫,执行新的全局前置守卫，意思就是会再次调用beforeEach

**结论：next()是放行，不会引发beforeEach再次调用，next('xx')带参数则会携带参数重新执行路由守卫**
...
如果 require参数含有表达式(expressions)，会创建一个上下文(context)，因为编译时(compile time)并不清楚具体是哪一个模块被导入
...
新建一个js文件，使用[插件](https://cn.vuejs.org/v2/guide/plugins.html#ad)的方式进行一次性的全局注册
...
```js
// 这个require是webpack自己定义的对象，context方法指定路径和文件类型， 以便实现更细粒度的模块引入
const routeFiles = require.context('../router/main', true, /\.ts/)
// 返回的routeFiles是一个函数，它的prototype上有一个keys()方法，返回一个符合路径条件的数组
routeFiles.keys().forEach((key) => {
...
1.安装依赖 [svgo](https://cloud.tencent.com/developer/tools/blog-entry?target=https%3A%2F%2Fpipbest.cn%2F%3Fgolink%3DaHR0cHM6Ly9naXRodWIuY29tL3N2Zy9zdmdv) 和 svg-sprite-loader

2.vue.config.js配置svg-sprite-loader

**配置文件：vue.config.js （基于webpack-chain插件）**

```js
config.module
      .rule('svg')
      .exclude.add(resolve('src/icons'))
      .end() // 这条配置是指针对名为svg的规则，排除掉src/icons下的文件
...
// 父组件需要监听update事件，并更新数据，通过$event接收传递的值
<child-component :visible="visible" @update:visible="visible = $event" />
// 或者使用函数接收
<child-component :visible="visible" @update:visible="val => visible = val" />
```
...
// 父组件使用.sync修饰符 等同上面的update监听事件
<child-component :visible.sync="visible" />
```
...
// 取消请求时手动调用（message 参数是可选的）
source.cancel('Operation canceled by the user.'); // network中网络请求状态会变成canceled
```
...
devDependencies 指开发环境所需要的依赖，类似eslint,babel都是编译时用到的，生产环境用不到

dependencies则是生产环境所需要的依赖

通过npm install xx -D 安装到devDepdencies，npm install xx -S  (不写-S亦可) 安装到dependencies



tree shaking的出现，使得一些没有被用到的包根本不会被打进最终产物。

**所以即使我们在dependencies中写了开发环境所需的包，但只要我们的代码中没有引入他，他都是不会被打包进最终产物的。**



区分目的：依赖库可能还依赖另外的库，当引入其他库时，只会去下载package.json中的dependencies的依赖，若放到了devDependencies中安装就会报错，所以在作为库使用时，必须把依赖放在指定位置上



结论：

**开发项目时依赖放哪都没影响，但是作为库提供给外部使用时需要严格分类（除非项目已经编译好并暴露了配置）。**
...
问题：受到浏览器同源策略限制，不同域名的页面之间无法共享cookie，常出现在子页面需要鉴权等问题



解决方案

1：使用postMessage API 在父页面中监听子页面的postMessage事件，实现跨域传递数据和cookie

2（常用）：利用nginx反向代理，将主子页面设置为同域。使用nginx可以将两个不同域的页面设置为同域，具体实现方式是使用nginx的反向代理功能，将两个不同的页面都代理到同一域名下，这样两个页面就可以共享cookie了，从而实现同域访问
...
<iframe src="/demo/iframe/iframe.html" name="iframe_a" width="600" height="400"></iframe>

    
<!--点击这个链接后，iframe就会显示这个网站-->
...
5、app系统拿到ST后，从后台向SSO发送请求，验证ST是否有效。**(用st作为参数调用验证接口，验证通过后登录)**
6、验证通过后，app系统将登录状态写入session并设置app域下的Cookie。



===至此，跨域单点登录就完成了。以后我们再访问app系统时，app就是登录的。接下来，再看访问app2系统时的流程。===
...
4、app2拿到ST，后台访问SSO，验证ST是否有效。 **(用st作为参数调用验证接口，验证通过后登录)**
5、验证成功后，app2将登录状态写入session，并在app2域下写入Cookie。



### path/publicPath/contentBase

output.path ==> 打包文件存放的位置，是一个绝对路径

output.publicPath ==> 打包文件index.html引用资源的前缀路径，为所有资源指定一个公共路径

**index.html本身也是一个资源，index内引用的资源都是一个个请求，会被请求下来**

publicPath设置为/，引用资源指向根路径，访问localhost:xx/即可请求到打包后的index.html

**如果设置非根路径如为/aaa/，contentBase默认值是项目根目录**

- 情况1：和devServer.publicPath路径一样，请求根路径/ 时，contentBase默认指向项目根目录，所以默认请求了根目录的索引文件原始index.html，和它所引用的静态文件，而请求localhost:xx/aaa/ 才会请求打包后的index.html（挂载了<script src="/aaa/app.js"></script>>这个节点的index.html）

- 情况2：和devServer.publicPath的路径不同， 那么请求/aaa/ 会请求一个原始index.html资源和它内部的静态资源



devServer.publicPath ==> 本地服务器打包生成的静态文件所在的位置，若 devServer.publicPath没有设置，则会取 output.publicPath的值

devServer.publicPath优先级高于output.publicPath, 若devServer设置了不同的publicPath, 则访问devServer.publicPath路径的index中引用的静态资源是404，因为index中引用的静态资源还是output.publicPath的路径

**个人理解：访问devServer.publicPath的路径，访问的是真正打包后的Index.html，会多挂一个<script src=app.js></script>节点， 而未打包的index.html没有这个节点**



devServer.contentBase ==> 不由webpack打包的静态文件的存放路径，即要访问的静态资源根目录，默认值：当前工作目录

**例：publicPath设置/，contentBase设置./public   即访问localhost:8080/  访问的静态资源是/public文件夹内的资源**
...
在有静态文件需要 serve 的时候必填，contentBase 不会影响 path和publicPath，它唯一作用就是指定服务器根目录来引用静态文件。

**contentBase 是服务于引用静态文件的路径，而 publicPath 是服务于打包出来的文件访问的路径，两者是不互相影响的。**



开发阶段：npm run dev启动本地服务的资源地址，默认是项目的根目录，也就是index.html所在目录

打包阶段：使用**CopyWebpackPlugin**插件，contentBase只对开发阶段有效

**PS：webpack5 起contentBase属性已弃用，可用static属性**

```js
module.exports = {
  devServer: {
    contentBase: './public', // 服务的引用路径，将静态资源目录指向./public，相对当前项目根目录
    publicPath: '/dist' // 打包输出来的文件访问路径，虚拟目录,资源路径的前面会添加这个路径
  },
}
```
...
resolve参数和then返回值是promise对象时的情况：**都是基于该promise对象状态改变而改变，值也等于该promise的resolve值**

```js
let p2 = new Promise(resolve => {
    resolve('p2res')
}).then(res => 'p2thenRes').then(res=> 'p2finalRes')
let p1 = new Promise(resolve => {
    // resolve传入promise对象p2，源码中会把下面then回调直接放到p2的then队列中等待p2状态改变后调用
    resolve(p2) 
}).then(res => console.log('final==>', res)) // final==> p2finalRes


let p4 = new Promise(resolve => {
    resolve('444')
}).then(res => '555').then(res => '666')
let p3 = new Promise(resolve => {
    resolve('123')
    // then中回调返回promise对象p4，下一个then会放到p4的then队列中等待p4状态改变后回调
}).then(res => p4).then(res => console.log('p3res==>', res)) // p3res==> 666
```
...
```js
// 前面加 ~ 表示后面的值为 alias, 然后就会去 webpack alias 配置中找相应的值, 然后拼接成最后的地址, 例如
configureWebpack: {
...
// 等同于
@import "cc/vv/bootstrap/dist/css/bootstrap.min.css";
@import "vv/bb/font-awesome/css/font-awesome.min.css";
```
...
**npx是npm的一个包执行器**，简化使用npm的步骤，从npm5.2起自带npx
...
npm安装和创建create-react-app

npm 会在本地全局性的安装 create-react-app，这个包会存储在 node 目录下面去。以后创建 react 项目直接执行 create-react-app 命令就可以了
...
npx创建creat-react-app

npx 命令他会把 create-react-app 安装包临时安装上，等项目初始化完成以后就删除掉。
...
```js
// npx支持运行远程仓库的可执行文件
npx github:puicco/cowsay hello
...
计算机文件在逻辑上基本上分为二种：二进制文件和 ASCII（也称[纯文本文件](https://baike.baidu.com/item/纯文本文件/4865229?fromModule=lemma_inlink)），图形文件及文字处理程序等[计算机程序](https://baike.baidu.com/item/计算机程序/3220205?fromModule=lemma_inlink)都属于二进制文件。这些文件含有特殊的格式及计算机代码。ASCII 则是可以用任何文字处理程序阅读的简单文本文件。

这其实是一种很糙的分类方式。文件是文本文件还是二进制文件，这是逻辑上的概念，而不是物理上的概念。**在物理上，文件都是基于二进制来存储的。**



文本文件

文本文件基于字符编码，常见为ASCII, unicode编码等，如果整个文件每个字节的内容能够被解码为字符数据，就认为这个文件是文本文件。也就是说，**文本文件是一种特殊的二进制文件。**



二进制文件

二进制文件也是通过二进制形式存储到文件中，但需要专门的解码软件处理，才能还原信息。

二进制文件是根据值编码的（指定某个值是什么意思，自定义编码）多少个比特代表一个值，完全由你决定。例如BMP文件，其头部是较为固定长度的文件头信息，前2字节用来记录文件为BMP格式，接下来的8个字节用来记录文件长度，再接下来的4字节用来记录bmp文件头的长度。
...
当在处理 [KeepAlive](https://vuejs.org/guide/built-ins/keep-alive.html) 组件时，我们通常想要保持路由组件活跃，而不是 RouterView 本身。为了实现这个目的可以将 KeepAlive 组件放置在插槽内：
...
<!--类似地，也可以用 Transition 组件来实现在路由组件之间切换时实现过渡效果：-->
<router-view v-slot="{ Component }">
  <transition>
    <component :is="Component" />
  </transition>
...
```vue
<router-view v-slot="{ Component }">
    <!--如果将引用放在 <router-view> 上，那引用将会被 RouterView 的实例填充，而不是路由组件本身。-->
...
export const user = defineStore('user', {
    ...
    // 持久化配置，把store的数据保存到localStorage中
    persist: {
    enabled: true, // 开启缓存
    strategies: [
      {
...
https://zhuanlan.zhihu.com/p/613022541





### JS形参顺序不固定写法（参数是object类型）
...
**为什么angular回调函数中有的形参必须写指定名称：比如\$scope, $http等，并且可以不按顺序传入？**

angular在执行回调前，会进行参数推导，并注入到当前模块中
...
angular.module()
      ...
      .injector.invoke(function(minus, plus, complex) { //angular 没有这个属性，为了展示方便这样写
        document.write(minus(3, 1)+"<br />");
        document.write(plus(3, 1)+"<br />");
        document.write(complex(3, 1)+"<br />");
      });


// 原理解析(简化版)
function extractArgs(fn) {
    // toString返回fn函数的源码字符串，通过正则匹配获得形参列表
    var args = fn.toString().match(/^[^\(]*\(\s*([^\)]*)\)/m); 
    return args[1].split(',');
}

function CreateInjector(caches) {
    this.caches = caches
}

CreateInjector.prototype.invoke = function(fn, self) {
    let argString = extractArgs(fn) // 获取形参列表
    let args = []
    argString.forEach(val => {
        /* 这里this指向调用者，即实例injector
        caches['minus'], caches['plus'] 在modules['complex']调用前已被赋值，就是modules['minus'] modules['plus']
        所以modules['complex']传入的minus和plus形参可以直接使用
        */
        args.push(this.caches[val.trim()]) 
    }, this)
...
angular.module = function() {
    let modules = {}
    let injector = new CreateInjector(modules) // 用module创建一个injector对象
    return {
        injector: injector,
        factory: function(name, fn) {
            // modules['minus'] = function(a,b){...}
            // modules['plus'] = function(a,b){...}
            // modules['complex'] = function(a,b){...}
            modules[name.trim()] = this.injector.invoke(fn)
            return this // 返回整个对象，满足链式调用
        }
    }
}
```
...
SSH是一种网络协议，通过加密和认证机制实现安全的访问和文件传输等业务；SSH是替代Telnet、FTP，使用明文传输数据的网络协议方案。
...
密钥认证：使用公钥私钥对进行身份验证，基本原理是服务器端使用客户端的公钥对随机内容加密，客户端使用自己的私钥解密并发送给服务器以证实自己的身份，实现免密登录

![img](D:\typora-img\v2-ad1496bf71ffa3b0cc28a8833a7c781c_1440w.webp)
...
安装volar插件后，template中标签补全失效，手动解决：settings-->Text Editor--> Files --> AddItem: Item填：*.vue ； Value填html
...
### vue-router 的配置项 base

前端部署在非根目录下时需要配置（且为history模式）
...
binding：一个对象，包含以下属性：
	name：指令名，不包括v-前缀。
	value：传递给指令的值。
	oldValue：之前的值，仅在update和componentUpdated钩子中可用。
	expression：绑定的表达式。
	arg：传给指令的参数。
	modifiers：一个包含修饰符的对象。
vnode：虚拟DOM节点。
oldVnode：上一个虚拟DOM节点，仅在update和componentUpdated钩子中可用。
...
**`JSON.stringify()` 永远都会转义双引号和反斜杠（生成 `\"` 和 `\\`），这是标准行为。**

你“看不到”反斜杠，是因为 **浏览器开发者工具在打印时做了美化显示**，让你误以为没有转义。

而接口传参时，你看到的是 **原始的、真实的 JSON 字符串**，所以看到了 `\"`。

**如何避免混淆**

| 场景               | 建议                                                 |
| ------------------ | ---------------------------------------------------- |
| 调试对象结构       | `console.log(obj)` 直接打印对象                      |
| 调试 JSON 字符串   | `console.log(JSON.stringify(obj, null, 2))` 美化输出 |
| 查看真实字符串     | 使用 `alert()` 或 `Network` 面板                     |
| 避免双重 stringify | 确保 `body` 只被 stringify 一次                      |



### URLSearchParams

**操作 URL 查询参数**（即 URL 中 `?` 后面的部分）。

它既可以用于**构建**查询字符串，也可以用于**解析和读取**现有的查询字符串。

```js
// 1. 从查询字符串创建
const params1 = new URLSearchParams('name=Alice&age=25&city=Beijing');

// 2. 从对象创建（注意：对象值会自动转为字符串）
...
// 3. 从数组创建（键值对数组）
const params3 = new URLSearchParams([['name', 'Charlie'], ['score', '95']]);

// 4. 从另一个 URLSearchParams 实例创建（复制）
const params4 = new URLSearchParams(params1);

// 添加一个键值对。允许重复的 key
params.append('tag', 'js');
params.append('tag', 'web'); // 可以重复添加
// 结果: tag=js&tag=web

// 获取第一个匹配key的值
const name = params.get('name'); // "Alice"
const missing = params.get('unknown'); // null

//获取所有匹配 key 的值，返回一个数组。
params.append('tag', 'js');
params.append('tag', 'react');
const tags = params.getAll('tag'); // ['js', 'react']

//检查是否包含某个 key
if (params.has('name')) {
    console.log('包含 name 参数');
}

// 将参数对象转换为标准的查询字符串
params.toString(); // "name=Alice&age=25&tag=js&tag=react"
...
// 使用body URLSearchParams实例是 Body 的合法输入，可以直接作为 fetch 的 body
fetch('/login', {
    method: 'POST',
    headers: {
...
// 使用场景2 解析页面URL参数
// 假设当前 URL 是: https://example.com/page?name=Alice&age=25
