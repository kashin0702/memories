...
output.publicPath ==> 打包文件内index.html引用资源的前缀路径，为所有资源指定一个公共路径

**个人理解：index.html本身也是一个资源，index内引用的资源都会被请求下来，如果设置非根路径如为/aaa/**

- 情况1：和devServer.publicPath路径一样，请求根路径/ 请求的是原始Index.html和引用的静态文件，请求/aaa/ 才会请求打包后的index.html

- 情况2：和devServer.publicPath的路径不同， 那么请求/aaa/ 会请求一个未打包app.js的原始index.html和它内部的静态资源



devServer.publicPath ==> 本地服务器打包生成的静态文件所在的位置，若 devServer.publicPath没有设置，则会取 output.publicPath的值

devServer.publicPath优先级高于output.publicPath, 若devServer设置了不同的publicPath, 则index中引用的静态资源是404，因为index中引用的静态资源还是output.publicPath的路径
...
打包阶段：使用**CopyWebpackPlugin**插件，且可以把contentBase设置为false
...
module.exports = {
  devServer: {
    contentBase: './public', // 服务的引用路径，将静态资源目录指向./public
