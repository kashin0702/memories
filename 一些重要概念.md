...
```js
//3. this的指向在函数创建时是无法决定的，this仅指向调用它的上一级对象
var obj = {
    a: 'haha',
    fn: function(){
		console.log(this.a)  
...
export default {
  name: 'cartList',
  computed: {
      //利用辅助映射函数导入vuex--store-getters内的cartList
    ...mapGetters(['cartList'])   
  },
...
### new FileReader()- 原生读取文件

这块主要涉及在前端处理图片上传时候回显、接收后端文件
...
1. URL.createObjectURL() 静态方法会创建一个 DOMString，其中包含一个表示参数中给出的对象的URL。

2. 这个 URL 的生命周期和创建它的窗口中的 document 绑定。这个新的URL 对象表示指定的 File 对象或 Blob 对象。
...
2. 使用 File 或 Blob 对象指定要读取的文件或数据。
   ...
   **异步读取指定的Blob中的内容，一旦完成，会返回一个`data:` URL格式的字符串以表示所读取文件的内容。**
