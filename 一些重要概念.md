...
句点法获取属性时，获取的是对象的key，而不是变量,**如obj.name  这个name只能是obj中名为’name‘的key，不能是变量名**
...
- 箭头函数的 this和**调用时上下文无关**，它没有自己的this, 永远指向其上下文的  this ，一旦确定，任何方法都改变不了其指向，如 call() ,  bind() ,  apply() 
