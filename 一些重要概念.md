...
arr.filter((item, index) => {
    //indexOf只会返回匹配到的第一个数组下标， 所以重复的都不会匹配到
...
function mul (arg1, arg2) {
    ...
    // 求2个数的小数点后位数之和
	try {m += s1[1].length} catch(e) {} 
    try {m += s2[1].length} catch(e) {}
    // 两数去掉小数点相乘， 再除以两数的平方和
...
```js
let x = true
// 1.先计算===  2.再计算|| 即： ( true || false) ? 'ddd' : 'xxx'  
let y = x || 123 === 456 ? 'ddd' : 'xxx'  // 结果：y='ddd'
